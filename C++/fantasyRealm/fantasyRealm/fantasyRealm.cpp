...
int main() {
	...
	while (1)
	{
		...
		while (!bDead)
		{
			...
			wsprintf(&screen[nScreenWidth + 5], L"%ls", wname.c_str());

			// Draw player
			screen[player.y * nScreenWidth + player.x] = bDead ? L'X' : ch;
...
wstring start() {
	...
	wstring wname = converter.from_bytes(name);
	const wchar_t* cname = wname.c_str();
