	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				wchar_t s[256];
				swprintf_s(s, 256, L"fantasyrealm - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
				SetConsoleTitle(s);
...
protected:
	...
	HANDLE m_hConsole;			// output buffer (mouse events)
	HANDLE m_hConsoleIn;		// input buffer
